...
class Pseudos(tk.Toplevel):
    def __init__(self, root, app, nombre_joueurs):
        ...
        self.canvas.create_text(250, 50, text="Entrez les pseudonymes des joueurs", font=("Comic sans ms", 14))

        # pour chaque joueur selectionner la couleur et entrer un pseudo
        for i in range(self.nombre_joueurs):
            y = 100 + i * 80
            self.canvas.create_text(100, y, text=f"Joueur {i+1}:", font=("Comic sans ms", 12), anchor="w")
        ...
        if all_filled:
            for i, entry in enumerate(self.entries):
                if entry.winfo_exists():
                    ...
                    self.info_joueurs[i] = [name, self.colors[i], 0, 0, False, False, True]
