...
class Victoire(tk.Toplevel):
    def __init__(self,root, app, joueur_gagnant):
        super().__init__(root)
        self.app = app
        ...
        image_sortie = Image.open("troll_farceur/images/sortie_ia.png")
        image_sortie = image_sortie.resize((500, 700), Image.LANCZOS)
        self.photo_sortie = ImageTk.PhotoImage(image_sortie)

        self.canvas.create_image(0,0,anchor="nw", image=self.photo_sortie)
        self.canvas.create_text(200, 50, text=f"Le joueur {joueur_gagnant} a gagné !", font=("Comic sans ms", 18), fill="white")
        self.creer_boutons()
        self.bind_events()

        pygame.mixer.init()
        pygame.mixer.music.load("troll_farceur/son/Victoire.mp3")
